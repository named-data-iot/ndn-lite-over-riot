 ...
 *  2. Install identity certificate named /ndn-iot/<service>/<identity>/KEY/234/home/567
 *  3. [Optional] Add route /ndn-iot to the given <face>
 *  4. Install KeyID-10002 Key as default AES-128 Key.
 */
void
simulate_bootstrap(ndn_face_intf_t* face, uint8_t service, uint8_t is_controller, name_component_t* identity, uint32_t identity_size, uint8_t add_route)
{
  ...
  ndn_ecc_prv_init(self_prv, secp256r1_prv_key_str, sizeof(secp256r1_prv_key_str),
                   NDN_ECDSA_CURVE_SECP256R1, 1412501962);
  ...
  ndn_name_append_bytes_component(&self_cert.name, &service, sizeof(service));
  if (is_controller) {
    ndn_name_append_string_component(&self_cert.name, "CMD", strlen("CMD"));
  }
  else {
    ndn_name_append_string_component(&self_cert.name, "DATA", strlen("DATA"));
  }
